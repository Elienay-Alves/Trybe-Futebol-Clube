...
class Data {
  ...
  private homeCalculation() {
    ...
    for (let index = 0; index < inHomeGames.length; index += 1) {
      ...
      this.getResult(homeTeamGoals, awayTeamGoals);
    }
  }

  private awayCalculation() {
    ...
    for (let index = 0; index < inAwayGames.length; index += 1) {
      ...
      this.getResult(awayTeamGoals, homeTeamGoals);
    }
  }

  private getResult(goalsFavor: number, goalsOwn: number) {
